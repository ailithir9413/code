...
my %lookup_hash = (
					assignment 	=> 'dropdown',
					biotic 		=> 'checkbox',
					bottom_type => 'checkbox',
					sediment	=> 'checkbox',
					);

my %options = ();
foreach my $key (sort keys %lookup_hash) {
	$options{$key} = construct_options::get_options($lookup_hash{$key}, $key);
	}
...
<div class="percent 
 ...
 <br>
 <label for="percent">Sand Percent:    </label><input id="percent" type="number" name="sand_percent" min="5" max="100" step="10" value ="5"/>
 <br>
 <label for="percent">Coarse Shells    </label><input id="percent" type="number" name="coarse_shells_percent" min="5" max="100" step="10" value ="5"/>
 </div>
